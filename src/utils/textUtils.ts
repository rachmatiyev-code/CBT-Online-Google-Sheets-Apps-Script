...
export function cleanQuestionText(raw: string): string {
  ...
  // "Seorang peserta didik mempelajari operasi hitung (penjumlahan, pengurangan, perkalian, pembagian). "
  cleaned = cleaned.replace(
    /^Seorang\s+peserta\s+didik\s+mempelajari\s+(?:operasi\s+hitung\s*)?(?:\([^)]*\)\s*)?[^:\n\-–—.]*[:\-\u2013\u2014.]+\s*/i,
    ''
  );

  // Also catch generic "Seorang peserta didik mempelajari..." ending in dash, colon, or period
  cleaned = cleaned.replace(
    /^Seorang\s+peserta\s+didik\s+mempelajari\s+[^.?!]+(?:[:\-\u2013\u2014]+|\.\s+)(?=[A-Z0-9])/i,
    ''
  );
  ...
  // 4. Strip leftover leading dashes, colons, dots, or whitespace
